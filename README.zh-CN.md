...
- 提供管理员/客户登录，并使用 access token + refresh token 会话
- 记录管理操作审计日志
...
PostgreSQL 里仍然保存精确的 byte 计数，但 API JSON 响应会额外带上十进制 GB
辅助字段：`upload_gb`、`download_gb`、`total_gb`。

域名访问分析和 Xray `StatsService` 是两条链路。StatsService 只提供流量计数，不提供
访问过的 hostname。控制面现在有 `domain_access_logs` 表和管理端写入/查询/汇总接口，
后续可以在节点侧加 Xray access-log shipper，把清洗后的 domain-only 事件送进来。
不要发送完整 URL、path、query string 或请求 body。
...
internal/security/        登录 token、订阅 token、密码校验、哈希
...
的管理员密码、少于 32 个字符的 secret key，或缺失/无效的
`PCP_DATABASE_ENCRYPTION_KEY` 启动。

管理员和客户登录都会返回一组短期 access token 和长期 refresh token。
access token 有效期由 `PCP_ACCESS_TOKEN_EXPIRE_MINUTES` 控制，默认 30 分钟；
refresh token 有效期由 `PCP_REFRESH_TOKEN_EXPIRE_HOURS` 控制，默认 refresh 窗口是 15 天。
refresh token 是 32 字节安全随机值，PostgreSQL 只保存它的 SHA-256 哈希。每次
refresh 都会返回新 refresh token，并撤销旧 refresh token。
管理员 refresh token 还会绑定一个内部 session version，这个版本由管理员邮箱、
管理员密码、`PCP_SECRET_KEY` 和可选的 `PCP_ADMIN_SESSION_EPOCH` 派生。修改其中
任意一个值都会让已有 admin refresh token 失效；如果只是想强制所有管理员重新登录，
...
subscription token 仍然用 `token_hash` 验证，同时会把 token 明文用 AES-256-GCM
加密后存入数据库字段，方便未来客户 App 登录后重新展示订阅链接。旧的 hash-only token
仍然能用，但除非 rotate 或重建，否则无法恢复明文。上面的示例值是故意放的假 key，
真实部署前一定要在 `.local/app.env` 里换成你自己生成的值。
...
`traffic_usage_daily`，并按各自 retention 清理 `domain_access_logs`、
`auth_refresh_tokens` 和 `audit_logs`。PostgreSQL 删除后物理文件不一定立刻变小，
但释放出的空间会被后续写入复用。
...
`POST /admin/login` 和 `POST /customer/login` 返回：
...
`POST /auth/refresh` 接收 `{"refresh_token":"..."}`，并返回新的 access token
和新的 refresh token。`POST /auth/logout` 接收同样的 body，用来撤销这个
refresh token。

管理员接口需要带 `admin` role 的 access token。客户接口需要带 `customer`
role 的 access token：
...
access token 的 JWT payload 会尽量少放信息。管理员 token 使用
`sub=configured-admin`；客户 token 使用 `sub=<customers.id>`。客户邮箱和管理员
邮箱只会在登录响应的 `principal` 对象里返回给前端展示，不会写进 JWT。
